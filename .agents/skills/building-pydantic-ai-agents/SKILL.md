...
- User asks about hooks, lifecycle interception, or agent runtime debugging
...
- **Debugging**: Use deterministic model overrides and `capture_run_messages()` to inspect failed runs without coupling the project to a specific telemetry backend.
...
| Testing, request inspection, and runtime debugging | [Testing and Debugging](./references/TESTING-AND-DEBUGGING.md) |
